                ...
                <div className="flex items-center gap-3 w-full">
                    ...
                    <div>
                        <h2 className="text-sm font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                           ...
                           <span className="px-2 py-0.5 rounded-full bg-accent/15 text-accent text-xs font-medium">Enterprise RAG</span>
                        </h2>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6 scroll-smooth space-y-6 flex flex-col pt-0 mt-4 custom-scrollbar pattern-cross pattern-gray-100 dark:pattern-gray-900 pattern-size-4 pattern-opacity-40">
                {messages.length === 0 ? (
                    <div className="flex-1 flex flex-col items-center justify-center text-center max-w-sm mx-auto p-8 opacity-60">
                         ...
                         <p className="text-sm text-gray-500 mt-2">Team channel initialized securely. Type <span className="text-accent font-mono font-bold bg-accent/10 px-1 py-0.5 rounded">@KnowledgeBot</span> to ask AI questions based on your restricted data.</p>
                    </div>
                ) : (
                            ...
                            <div key={msg._id || index} className="flex justify-start w-full">
                                <div className="max-w-[85%] flex gap-4">
                                    <div className="flex-none pt-1">
                                        <div className="w-8 h-8 bg-accent-gradient rounded-xl flex items-center justify-center shadow-accent">
                                                    ...
                                                    <div className="flex flex-wrap gap-2">
                                                        {msg.sources.map((src, i) => (
                                                            <div key={i} className="flex items-center gap-1.5 px-2.5 py-1.5 bg-gray-50 dark:bg-[#1a1a1a] border border-gray-100 dark:border-white/5 rounded-lg">
                                                                <span className="text-[10px] font-mono text-accent truncate max-w-[120px]">
                    ...
                    <div className="flex justify-start w-full animate-in slide-in-from-bottom-2 fade-in duration-300">
                         <div className="max-w-[85%] flex gap-4">
                             ...
                             <div className="flex flex-col flex-1 pl-1">
                                <span className="text-xs text-accent font-medium tracking-wide">KnowledgeBot is synthesizing documents...</span>
                             </div>
                         </div>
                    </div>
                )}
            ...
            <div className="flex-none p-4 md:p-6 bg-white dark:bg-[#0A0A0A] border-t border-gray-100 dark:border-white/10 z-10 w-full lg:max-w-4xl lg:mx-auto">
                <form onSubmit={handleSendMessage} className="relative flex items-center">
                    <input
                        ...
                        className="w-full bg-gray-50 dark:bg-[#1a1a1a] text-gray-900 dark:text-white text-sm rounded-full pl-6 pr-14 py-3.5 focus:outline-none focus:ring-2 ring-accent/50 border border-gray-200/50 dark:border-white/5 transition-all shadow-sm"
                        disabled={isBotTyping}
                    />
                    <button
                        type="submit"
                        disabled={!newMessage.trim() || isBotTyping}
                        className="absolute right-2 p-2 bg-accent-gradient text-white rounded-full hover:opacity-90 disabled:opacity-50 transition-all shadow-accent"
                    >
